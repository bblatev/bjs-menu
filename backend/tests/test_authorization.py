...
class TestStaffCannotAccessManagerEndpoints:
    ...
    def test_staff_cannot_delete_menu_item(self):
    ...
    def test_staff_cannot_access_financial_reports(self):
        headers = _token("staff")
    ...
    def test_staff_can_access_menu_read(self):
        headers = _token("staff")
...
class TestManagerCannotAccessOwnerEndpoints:
    ...
    def test_manager_can_read_staff(self):
...
class TestUnauthenticatedDenied:
    ...
    def test_no_token_on_protected_get(self):
        resp = client.get("/api/v1/financial/accounts/")
        assert resp.status_code == 401

    def test_no_token_on_protected_post(self):
        resp = client.post("/api/v1/staff", json={"name": "test"})
        assert resp.status_code == 401

    def test_no_token_on_protected_delete(self):
